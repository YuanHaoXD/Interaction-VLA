...
- **权威文档**：`docs/2026-07-14-交互VLA设计文档_zh.md` 是唯一权威总设计，与任何其它文档冲突时以它为准。任务书是 `docs/plans/2026-07-14-M1地基实施计划.md`（当前阶段 M1）。
...
用户文本 + 摄像头帧(≈1fps) ─▶ JoyAI 8B(慢脑,1Hz,冻结) ─▶ 文本回复
                                     │ 决策token隐状态(训练期离线预计算)
                                     ▼
                            动作专家(快脑,30Hz) ─▶ 动作块 ─▶ 安全层 ─▶ set_target
```

三个必须理解的核心洞察（设计文档 §2，驱动全部设计）：
1. **节奏差桥接 = action chunking**：慢脑 1Hz、控制需 30Hz。每个决策步输出未来 1.5s 动作块（45步×9维），块间重叠区 temporal ensembling 混合 → 天然连续无跳变。
...
### 动作表示（"字母表"，改动即大事）
9 维任务空间向量 @30Hz：`[x, y, z, roll, pitch, yaw, body_yaw, ant_R, ant_L]`（头部平移/姿态 + 身体旋转 + 双天线）。维度索引在 `data_gen/templates.py` 顶部注释；安全范围/限速在 `sim/limits.json`（B1 实测产出）。

### 数据契约 C0（schema v1，改动即大事）
每个 episode 是一个目录：`meta.json` + `timeline.json`（时间升序事件列表，`type ∈ {user_text, response, silence, delegate}`）+ `actions.npy`（float32 `[T,9]`，`T = round(duration_s*30)`）。可选：`hidden_states.npy`（`[N_decisions, d_model]`）、`frames/`、`audio_user.wav`（预留）。`response` 事件必须带 `est_speech_dur_s`。schema 的代码化在 `data_gen/schema.py`（`write_episode` / `validate_episode`）——任何数据生产/消费都应过 `validate_episode`。
...
- `expert/hidden_export/` —— 慢脑侧。`run_joyai.py`（JoyAI transformers 慢路径加载/推理，加载方式抄官方 `services/webinfer`，**别自己发明**）；`precompute.py`（隐状态离线预计算，当前是**简化版**：决策 token 位置用均匀采样占位，精确定位是 M2 待优化项，见文件内 TODO）。
...
- **A3（隐状态离线预计算）是全项目头号判定点**：若受阻，Plan B 是「回复文本 → 小型文本编码器」（设计文档 §3.3，接口不变），代价是丢失视觉上下文与沉默秒语义。当前简化版已跑通、判定通过。
