...
### ①-补:"Reachy Mini Control" 是什么?为什么它会让你连不上?(重要,踩坑高发区)

**Reachy Mini Control 就是官方的安装包 / 桌面 GUI 应用。** 它的作用是:自带一个 Python venv + SDK + `reachy-mini-daemon.exe`,并提供一个图形界面帮你**启动和管理 daemon**。你本机的 `reachy-mini-demo\.venv` 里那个 daemon.exe,就是它(或等价安装)装出来的。

**官方文档说"需要 Reachy Mini Control 去连接",意思是"连接靠它自带的 daemon"——不是要你额外装/连一个新东西。** 而我们用命令行 `reachy-mini-daemon --sim` 起 daemon,**本质就是在做 Reachy Mini Control GUI 做的同一件事**(启动 daemon 当机器人本体)。

> ⚠️⚠️ **最关键的一条:GUI 和命令行 daemon 是"二选一",绝对不能同时开!**
> 
> Reachy Mini Control GUI 和你命令行起的 `reachy-mini-daemon --sim`,**都想当"机器人本体"、都想占用 `localhost:8000` 端口**。同时开 = 两个 daemon 抢同一个端口 = 连接管线打架 = **你的脚本卡在 `>>> 连接仿真...` 连不上**。
> 
> **典型症状**(真实案例):动作脚本卡住不动,只打印:
> 
> ```
> >>> 连接仿真(首次可能等 10~50 秒)...
> Cannot change resolution of Mujoco simulated camera for now.   ← 这行是正常信息,不是错误
> No Reachy Mini Audio USB device found!                          ← 这行也是正常信息
> ```
> 
> 然后就卡住。用 `netstat -ano | findstr :8000` 会看到一堆 TIME_WAIT,进程里同时有 `reachy-mini-control.exe` 和命令行 daemon。
> 
> **解决:关掉其中一个,只留一个。** 推荐关掉 Reachy Mini Control GUI,只用命令行 daemon(和本文流程一致、更可控)。关 GUI 后,那个卡住的脚本也要一并关掉重跑(它卡在坏连接里不会自愈)。

**记住:本文所有流程只用命令行 daemon。跑之前先确认 Reachy Mini Control GUI 没开着。**
...
**⚠️ 第 0 步之二——确认 Reachy Mini Control GUI 没开着!** 如果你之前用那个桌面应用启动过机器人,先把它整个退出(见"①-补"节:它和命令行 daemon 二选一,同开必冲突)。本文全程只用命令行 daemon。
...
**预期:** 脚本先打印"连接仿真(约 4~8 秒)",连上后 3D 窗口里机器人依次做:点头 → 摇头 → 歪头 → 摆天线 → 转身 → 看四方 → 呼吸,共约 26 秒。

> **关于连接时间:** 脚本已按官方最佳实践配置(`connection_mode="localhost_only"` + `media_backend="no_media"` + `with` 上下文管理器),连接稳定在 **4~5 秒**(见 `03-官方AGENTS.md收获*.md`)。若你写自己的新脚本用了裸 `ReachyMini()`,连接会慢到 7~8 秒甚至更久且不稳定——**照文档 03 加上那两个参数**。等待时判断死活:看 3D 窗口机器人有没有动、或脚本有没有继续打印。
...
### 坑 0(头号坑!):脚本连不上,同时开着 Reachy Mini Control GUI

**这是最高发的坑,连不上时第一个排查它。** 见前面"①-补"节:GUI 和命令行 daemon 抢 8000 端口,导致脚本卡在 `>>> 连接仿真...`。

**一键诊断**(PowerShell):

```powershell
# 看是不是同时开了 GUI 和命令行 daemon
tasklist | findstr /i "reachy"
#   如果同时看到 reachy-mini-control.exe 和 reachy-mini-daemon.exe → 就是这个坑
netstat -ano | findstr :8000
#   看到一堆 TIME_WAIT → 连接管线已乱
```

**解决:**

1. 关掉 **Reachy Mini Control** GUI(整个应用退出,不只是最小化)。
2. 关掉卡住的动作脚本窗口(它卡在坏连接里,不会自愈)。
3. 命令行 daemon 窗口保留;重跑动作脚本即可。
4. 如果不确定谁是谁,干脆全关重来:`Get-Process python,reachy-mini-daemon,reachy-mini-control -EA SilentlyContinue | Stop-Process -Force`,然后回到"步骤 1"从头起命令行 daemon。

> **验证过**:关掉 GUI 冲突后,干净的命令行 daemon 立刻正常,脚本秒连、机器人点头。

### 坑 1:脚本一直停在"连接仿真...",好久不动

- **多半是正常的**——本机连接握手就是慢(10~52 秒)。先耐心等 1 分钟。
- 判断死活:3D 窗口机器人有没有动?没动但脚本没退出 = 还在连,继续等。
- **但如果超过 1 分钟**:先排查坑 0(有没有开着 GUI),再看坑 2。
