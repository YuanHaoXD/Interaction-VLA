...
def build_shows():
    # 仿真里幅度写大点更易观察(用户反馈);均在 B1 实测安全范围内(sim/limits.json)。真机部署须回 CALIBRATION 保守值。
    return [
        ("点头 nod        (pitch 正弦)",      _osc(4, 0.70, 2, 6)),
        ("摇头 shake      (yaw 正弦)",        _osc(5, 0.75, 1.2, 4)),
        ("歪头 tilt       (roll 保持)",       _hold(3, 0.60, 0.6, 1.0)),
        ("摆天线 wiggle    (双天线反相)",       _osc(7, 1.2, 3.0, 6) + _osc(8, -1.2, 3.0, 6)),
        ("转身 body_yaw   (身体左右转)",        np.concatenate([_hold(6, 0.9, 0.8, 0.6), _hold(6, -0.9, 0.8, 0.6)])),
        ("看向 look       (右→左→上→下)",      np.concatenate([
            _hold(5, 0.8, 0.5, 0.5), _hold(5, -0.8, 0.5, 0.5),
            _hold(4, -0.6, 0.5, 0.5), _hold(4, 0.6, 0.5, 0.5)])),
