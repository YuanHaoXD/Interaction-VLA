...
- 依赖图：`B1 → B2 → B3 → B4`；`A1、A2、A4 可并行先行`；`A3 依赖 A2`；`A5 依赖 B4（模板库+schema 已合入 git）`；`A6 依赖 B4`。
...
episode_<8位id>/
  ...
  audio_user.wav      # 可选 16kHz 单声道（A6 之后必需）
...
- [ ] **Step 3：写勘察报告**（`docs/experiments/M1-B1-动作空间勘察报告.md`：每维安全范围表、达成频率、观察到的异常、对 max_vel 的修订值）
...
- [ ] **Step 4：跑测试确认通过**（同 Step 2 命令，预期 4 passed）
...
- [ ] **Step 2：跑测试确认失败**（`ModuleNotFoundError: data_gen.schema`）
...
- [ ] **Step 5：起 sim daemon，肉眼回放一段**：先用 B2 模板生成一段临时 episode（下个任务会正式做），或直接跳过留到 B4。
- [ ] **Step 6：Commit**
...
- [ ] **Step 2：运行**（必须在仓库根、用 `-m` 方式，否则 `data_gen` 导入失败）：
...
- [ ] **Step 3：起 sim daemon，抽查回放 3 段**（`python sim\replay_episode.py samples\ep_00000000` 等），肉眼确认：有呼吸感、事件处动作明显、无跳变、无撞限位。观感问题记进 `docs/experiments/M1-B4-样例观感记录.md`（模板参数的调整会成为常态，记录基线很重要）。
- [ ] **Step 4：Commit**
...
- [ ] **Step 1**：定位服务器上 JoyAI 权重与代码（toy SFT 用过，环境在）；加载方式**抄 JoyAI 仓库 `services/webinfer` 的模型加载代码**（那是官方用法，别自己发明）；若 webinfer 走 vLLM，则用其模型名+`transformers.AutoModelForCausalLM/AutoProcessor(trust_remote_code=True)` 重建慢路径。
- [ ] **Step 2**：按 `datasets/README.md` 的转换格式构造一段 10 秒的每秒对话（`<t seconds>\n<image>` + `</silence>`/`</response>`），用数据集里任一真实样本的帧。
- [ ] **Step 3**：`model.generate(max_new_tokens=32)` 跑最后一秒，打印输出。预期：以 `</silence>` 或 `</response>` 开头。
- [ ] **Step 4**：测单卡吞吐（决策/秒），写报告 + Commit：
...
- [ ] **Step 1**：对每个决策秒，前向到该秒 assistant 起始位置，取 **last-layer 在决策 token（`</silence>`/`</response>` 首 token）处的 hidden state**。transformers 路径：`model(..., output_hidden_states=True)`，`hidden_states[-1][0, pos, :]`；一次前向可并取全部秒的位置（teacher forcing 整段跑一遍，按 assistant 起始位置索引切片——**不需要逐秒 generate**）。
- [ ] **Step 2**：在 B4 的 `samples/` 任取 10 段跑通（样例无帧则用纯文本+黑帧占位，报告中声明）；校验 `hidden_states.npy` 形状 = timeline 决策秒数 × d_model、无 NaN。
- [ ] **Step 3**：测吞吐（段/分钟·卡），估算 10 万段全量预计算的卡时，写报告。
- [ ] **判定**：开工 5 个工作日内 Step 2 未通 → 停止此路线，在报告写明卡点，启动设计文档 §3.3 的**文本 Plan B**（通知本地，本地会出 Plan B 补充计划）；已通 → 全量预计算排到 A5 数据就绪后台跑。
- [ ] **Commit**：`git commit -m "M1-A3: 隐状态离线预计算工具+吞吐评估"`
...
- [ ] **Commit**：`git commit -m "M1-A4: SDK/电机适配研究笔记"`
...
- [ ] **Step 1（TDD）**：先写 `tests/test_pipeline.py`——用 `datasets/example.json` 的第一个样本调 `convert_sample(sample, rng, limits) -> (meta, timeline, actions)`，断言：timeline 含全部 question（`user_text`）与 response（含 `est_speech_dur_s` 与 `action_label`）、actions 形状 = 视频秒数×30、`validate_episode` 通过。
- [ ] **Step 2**：实现 `convert_sample`：user_text 时刻 → speech_spans（跨度 = `est_dur(content)`，同一公式 5.5 字/秒中文、14 字符/秒英文）；带 action 标签的 response → events；`compose()` 生成轨迹；duration 取最后事件 + 5s 与视频长的较小者，上限 120s。
- [ ] **Step 3**：批量驱动：multiprocessing 扫标注文件目录（增量：已存在且校验通过的 episode 跳过），逐段 `validate_episode`，失败进隔离目录并计数。
...
**Files:**
- Create: `data_gen/synth_user_audio.py`
- Create: `docs/experiments/M1-A6-用户音频合成报告.md`

**Interfaces:**
- Consumes: A5 产出的 episodes；JoyAI 仓库 `services/tts`（Qwen3-TTS，vLLM-Omni）。
- Produces: 每段 `audio_user.wav`（16kHz mono）：对 timeline 中每条 `user_text` 用**随机音色/语速**合成并按 `t` 铺进整段静音底；meta.json 增写 `"user_audio": true` 与所用音色。

- [ ] **Step 1**：起 JoyAI 的 TTS 服务（照其 `services/tts/scripts/run.sh`），写批量合成脚本；音色池 ≥6、语速 0.85–1.2 随机。
- [ ] **Step 2**：先 100 段验证（听 3 段：位置对齐、无削波），再跑 10k 子集（其余排后台，M2 训练用多少合多少）。
- [ ] **Step 3**：报告 + Commit。
...
- [ ] `sim/limits.json` 实测产出，30Hz 达成率有结论（B1）
- [ ] 模板库+schema+回放器测试全绿（B2/B3）
- [ ] 100 段样例入库、肉眼回放合格（B4）
- [ ] NPU 三件套冒烟结论明确（A1）
- [ ] JoyAI 慢路径推理+吞吐数据（A2）
- [ ] 隐状态预计算通/不通判定完成；不通则 Plan B 已启动（A3）
- [ ] SDK 六问全答（A4）
- [ ] 数据集 v1 ≥10 万段 + stats.json（A5）
- [ ] 10k 段带用户音频（A6）
- [ ] `docs/experiments/` 每任务一篇中文记录，全部 push
