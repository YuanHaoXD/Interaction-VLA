...
def set_log_dir(log_dir: str):
    ...
    注意：这里**原地修改** _default_logger.log_dir，而不是重新绑定一个新的
    APILogger 对象。因为 tracker.py 的 _default_tracker 在 import 时就已经持有
    了对同一个 _default_logger 对象的引用；若在此处重新赋值模块级变量，tracker
    仍指向旧对象，set_log_dir 就会对 log_completion 静默失效（原文档的 latent bug）。
    原地改属性能让所有持有该对象引用的调用方都生效。
    """
    _default_logger.log_dir = Path(log_dir)
